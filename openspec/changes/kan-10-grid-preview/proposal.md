...
- Invalid or incomplete fields cannot be applied; Cancel and modal dismissal discard the draft. A successful Apply commits one `scene.setGrid` command, while a rejection leaves the form open with its error.
...
- The accepted grid still uses the existing `GridSpec`, `scene.setGrid` command and server authorization. No shared schema, event, persistence or API change is required.
