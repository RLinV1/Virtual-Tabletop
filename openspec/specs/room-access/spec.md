...
In the room, the GM SHALL see a remove action for each active player in the participants list. The GM SHALL also see a reset action next to the invite link. Both actions SHALL ask for confirmation before they take effect, and the remove confirmation SHALL name the player. The invite link shown SHALL be the server's current code, not a copy cached in the browser. Players SHALL NOT see either action. A revoked guest SHALL see a session-ended screen that says they were removed from the room. The browser SHALL forget its credentials for that room.
